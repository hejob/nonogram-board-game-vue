...
function isWin(currentChecks, targetCounts) {
  ...
  return (
    isEqualChecks(currentChecks.rowChecks, targetCounts.rowChecks) &&
...
function solveLineStatus(line, checks) {
  ...
  if (isEqualArray(currentLineChecks, checks)) {
    ...
    if (sum === 0) {
      ...
      status = 'NG'
    } else if (isLineEmpty(line) && (getMaxContinousLength(checks, r) <= l)) { // empty start, and free cell too many
      status = 'PS';
      console.log('SKIP');
      // no way to get any new solution at now, skips
      return { status, solutions: line.slice(0, n)}
    } else {
      if (l === 0) {
      ...
      if (matchCombination(line, candidate, n, solutions)) {
        ...
        matchAny = true
        if (l == 0) {
          status = 'PS' // as only one solution exists
        }
      ...
      if (status != 'NG') {
        ...
        while (generateCombinationNext(combination, l, t)) {
          triesCount += 1
          if (triesCount > 1e4) {
            // this is too big, we short-circuit it for now
            console.log('SHORTCIRCUIT! TOO LONG')
            status = 'PS'
            return { status, solutions: line.slice(0, n) }
          }
...
function getMaxContinousLength(checks, r) {
    // the maximum checks group length
    let m = 0;
    for (let i = 0; i < r; i++) {
        m = Math.max(m, checks[i]);
    }
    return m;
}
